...
mod test {
    ...
    fn test_bytes_to_u16_vec() {
        ...
        let u16_slice = bytes_to_u16_vec(&unicode_array.data).unwrap();
        let decoded_string = widestring::ucstr::U16CStr::from_slice(&u16_slice).unwrap().to_string().unwrap();

        assert_eq!(bytes_to_u16_vec(&unicode_array.data).unwrap(), &expected_u16_array);
        assert_eq!(&decoded_string, "Ø ƌëar");


        let empty_array = My16BitAlignedU8Array::new([]);
        assert_eq!(bytes_to_u16_vec(&empty_array.data).unwrap(), &[]);


        let odd_length_array = My16BitAlignedU8Array::new([1,2,3]);
        assert!(bytes_to_u16_vec(&odd_length_array.data).is_err());
    }
}
